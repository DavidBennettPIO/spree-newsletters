module NewsletterHelper
  ...
  def render_html_module newsletter_line, prev_module_name
    ...
    if ['h1','h2','h3','h4','h5','h6'].include?(newsletter_line.module_name)
      ...
      puts RAILS_ROOT + '/public/images/' + header_image + '.png'
      if FileTest.exist?(RAILS_ROOT + '/public/images/' + header_image + '.png')
        header = image_tag(header_image + '.png', :alt => header_text, :style => 'border: 0 ' + background_color + ' solid; border-width: ' + v_padding + ' 0;')
      elsif FileTest.exist?(RAILS_ROOT + '/public/images/' + header_image + '.jpg')
        header = image_tag(header_image + '.jpg', :alt => header_text)
      elsif FileTest.exist?(RAILS_ROOT + '/public/images/' + header_image + '.gif')
      ...
      output_html << content_tag(newsletter_line.module_name, nil, :style => 'margin: ' + hr_padding + '; border: ' + hr_color + ' 1px solid; border-width: 1px 0 0 0;' )
    else
